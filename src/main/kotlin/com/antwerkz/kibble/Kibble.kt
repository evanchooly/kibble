...
val FileSpec.classes: List<TypeSpec>
    get() = members
            .filterIsInstance(TypeSpec::class.java)
            .filter { it.kind is Class }
val FileSpec.interfaces: List<TypeSpec>
    get() = members
            .filterIsInstance(TypeSpec::class.java)
            .filter { it.kind is Interface }
val FileSpec.objects: List<TypeSpec>
    get() = members
            .filterIsInstance(TypeSpec::class.java)
            .filter { it.kind is Object }
...
fun TypeSpec.getClass(name: String): TypeSpec? = typeSpecs
        .filter { it.kind is Class }
        .firstOrNull { it.name == name }
fun TypeSpec.getObject(name: String): TypeSpec? = typeSpecs
        .filter { it.kind is Object }
        .firstOrNull { it.name == name }
