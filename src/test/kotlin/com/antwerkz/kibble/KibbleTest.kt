...
return "hi"""")
    ...
    fun sampleClass() {
        ...
        Assert.assertEquals(klass.properties[0].type, KibbleType(file, name = "Double"))
        Assert.assertEquals(klass.functions.size, 2)

        Assert.assertEquals(klass.functions[0].name, "output")
        Assert.assertEquals(klass.functions[0].parameters, listOf(KibbleParameter("count", KibbleType(file, name = "Long"))))
